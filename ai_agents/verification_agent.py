...
4. Are there any errors, contradictions, or misleading statements?

Provide one of these responses:
- "VERIFIED: [brief note]" if the response is accurate and helpful as-is
- "REVISED: [improved response]" if minor changes improve the response
- "UNVERIFIED: [reason]" if the response contains errors, inaccuracies, or doesn't adequately address the question

Be critical in your evaluation - only verify responses that are genuinely accurate and helpful."""
        ...
        verification_lower = verification_text.lower().strip()
        ...
        elif verification_text.startswith("UNVERIFIED:") or verification_text.startswith("REJECTED:"):
            return {
                "verified": False,
                "final_response": initial_response,
                "notes": verification_text.split(":", 1)[1].strip() if ":" in verification_text else "Response could not be verified"
            }
        elif any(word in verification_lower for word in ["incorrect", "inaccurate", "false", "wrong", "error", "problematic"]):
            return {
                "verified": False,
                "final_response": initial_response,
                "notes": "Response contains potential inaccuracies"
            }
        elif any(word in verification_lower for word in ["accurate", "correct", "good", "appropriate", "helpful"]):
            return {
                "verified": True,
                "final_response": initial_response,
                "notes": "Response verified as accurate"
            }
        else:
            return {
                "verified": False,
